...
@router.post("/", response_model=LeaveResponse, dependencies=[Depends(allow_roles(["Admin", "Employee"]))])
def create_leave(leave: LeaveCreate, db: Session = Depends(get_db)):
    return leave_crud.create_leave(db=db, leave=leave)

@router.get("/user/{user_id}", response_model=List[LeaveResponse], dependencies=[Depends(allow_roles(["Admin", "Employee"]))])
def read_leaves_by_user(user_id: int, db: Session = Depends(get_db)):
    leaves = leave_crud.get_leaves_by_user(db, user_id=user_id)
    return leaves

@router.get("/", response_model=List[LeaveResponse], dependencies=[Depends(allow_roles(["Admin"]))])
def read_all_leaves(db: Session = Depends(get_db)):
    leaves = leave_crud.get_all_leaves(db)
    return leaves

@router.patch("/{leave_id}", response_model=LeaveResponse, dependencies=[Depends(allow_roles(["Admin"]))])
